...
function exibeJogo() {
	...
	if (game.status_code < 200 || game.status_code > 300) {
		...
		document.querySelector(".titulo-jogo").innerText = `Game Não Encontrado`;
		return;
	}

	document.querySelector(".titulo-jogo").innerText = game.name;
...
xhrGame.open('GET', `${BASE_API_URL}games/${idGame}?key=${apiKey}`);
xhrGame.send();
